...
from bokeh.models import CrosshairTool, HoverTool, LinearAxis, Range1d
...
def plot_bokeh(
        ...
        spb[0].yaxis.axis_label = "RF mag [uT]"
        spb[0].extra_y_ranges = {"ang": Range1d(start=-math.pi, end=math.pi)}
        spb[0].add_layout(LinearAxis(y_range_name="ang", axis_label='RF angle [rad]'), 'right')
        ...
        for block_counter in range(len(seq.block_events)):
            ...
            if is_valid:
                ...
                if getattr(block, "adc", None) is not None:  # ADC
                    ...
                    # t = adc.delay + (np.arange(int(adc.num_samples)) + 0.5) * adc.dwell
                    # sps[1].vlines(x=t_factor * (t0 + t), ymin=-100, ymax=100, colors='b', linestyles=(0, (1, 10)))
                    # spb[1].block(x=[100], y=[-100], left=[t_factor * (t0 + t)], right=[adc], color='pink')
                ...
                if getattr(block, "rf", None) is not None:  # RF
                    ...
                    spb[0].line(t_factor * (t0 + time + rf.delay), np.abs(signal), line_width=2)
                    spb[0].line(
                        t_factor * (t0 + time + rf.delay),
                        np.angle(
                            signal
                        ...
                        #     * np.exp(1j * 2 * math.pi * time[ic] * rf.freq_offset)
                        # ),
                        line_width=2, y_range_name="ang", color='green'
                    )
                    ...
                    spb[0].cross(
                        ...
                        line_width=2, y_range_name="ang", color='red'
                    )
                ...
                for x in range(len(grad_channels)):  # Gradients
                    ...
                    if getattr(block, grad_channels[x], None) is not None:
                        ...
                        if grad.type == "grad":
                            ...
                            waveform = g_factor * grad.amplitude * np.array([0, 0, 1, 1, 0])
                        # sps[x+1].plot(t_factor * (t0 + time), waveform)
                        spb[x+1].line(t_factor * (t0 + time), waveform)
        ...
        for x in range(3):
            _label = grad_plot_labels[x]
            # sps[x+1].set_ylabel(f"G{_label} ({grad_disp})")
            spb[x+1].yaxis.axis_label = f"G{_label} ({grad_disp})"

        spb[-1].xaxis.axis_label = f"t ({time_disp})"
        # sps[-2].set_xlabel(f"t ({time_disp})")
